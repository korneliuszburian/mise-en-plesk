...
| Master and dev real-host proof | done | current checkout completed fresh bounded read-only scans through `pnpm --silent`: `master-ssh` reported 216 subscriptions and a 20-installation bounded page, while `dev-ssh` reported 92 subscriptions and a 20-installation bounded page; both SSH hosts were reachable, reports parsed as JSON, and the dev WP-CLI wrapper is now classified accurately as `wp-cli-missing` (`/usr/local/bin/wp: 404`) |
...
- `tests/cli-scan-e2e.test.ts` now exercises the producer CLI with isolated
  Bitwarden and SSH process doubles, asserting complete, budget-incomplete, and
  non-zero-offset behavior through the JSON report and heartbeat artifacts.
